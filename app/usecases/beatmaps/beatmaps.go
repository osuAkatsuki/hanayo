...
func GetBeatmapData(b string) (beatmap Beatmap, err error) {
	...
	resp, err := http.Get(settings.INTERNAL_BEATMAPS_SERVICE_BASE_URL + "public/api/b/" + b)
	if err != nil {
		return beatmap, err
	}
...
func GetBeatmapSetData(beatmap Beatmap) (bset BeatmapSet, err error) {
	settings := settingsState.GetSettings()
	resp, err := http.Get(settings.INTERNAL_BEATMAPS_SERVICE_BASE_URL + "public/api/s/" + strconv.Itoa(beatmap.ParentSetID))
	if err != nil {
		return bset, err
	}
	defer resp.Body.Close()
